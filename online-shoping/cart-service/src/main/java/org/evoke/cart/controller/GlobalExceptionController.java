...
public class GlobalExceptionController {
		...
		@ExceptionHandler(IllegalArgumentException.class) //delete
		...
		@ExceptionHandler(DataIntegrityViolationException.class) // update
	    public ResponseEntity<ErrorMessage> dataIntegrityViolationExceptionHandler(Exception ex) {
			ErrorMessage error = new ErrorMessage();
	        error.setErrorCode(ErrorCode.invalid_input_paramater);
	        error.setErrorDesc("invalid input paramater");
	        error.setErrorType(ErrorType.APPLICATION_PRACTICE_ERROR);
